...
class SegFormer(nn.Module):
    def __init__(self, backbone='b0', norm_layer=nn.BatchNorm2d, side_dims=3,
        ...
        if backbone == 'b0':
            ...
            self.backbone = mit_b0(side_dims=side_dims)
            ...
            self.backbone = mit_b3(side_dims=side_dims)
            self.decode_head = SegFormerHead(feature_strides=feature_strides, in_channels=in_channels, 
                                             num_classes=num_classes,in_index=in_index, dropout_ratio = dropout_ratio,
                                              decoder_params = decoder_params)
            ...
            num_classes = 1
            decoder_params=dict(embed_dim=512)
            self.backbone = mit_b4(side_dims=side_dims)
            self.decode_head = SegFormerHead(feature_strides=feature_strides, in_channels=in_channels,
                                             num_classes=num_classes,in_index=in_index, dropout_ratio = dropout_ratio,
                                              decoder_params = decoder_params) 
        elif backbone == 'b5':
            ...
            num_classes = 1
            decoder_params=dict(embed_dim=512)
            self.backbone = mit_b5(side_dims=side_dims)
            self.decode_head = SegFormerHead(feature_strides=feature_strides, in_channels=in_channels, 
                                             num_classes=num_classes,in_index=in_index, dropout_ratio = dropout_ratio,
                                              decoder_params = decoder_params)
    ...
    def forward(self, x, additional_features=None, points=None, coord_dist_map=None):
        with ExitStack() as stack:
            if self.inference_mode:
                stack.enter_context(torch.no_grad())

            features = self.backbone(x, additional_features)
            pred, feature = self.decode_head(features, points, coord_dist_map)
        return pred,feature
