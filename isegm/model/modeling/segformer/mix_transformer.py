...
class MixVisionTransformer(nn.Module):
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dims=[64, 128, 256, 512],
                 ...
                 attn_drop_rate=0., drop_path_rate=0., norm_layer=nn.LayerNorm, side_feature_early_fusion=False,
                 depths=[3, 4, 6, 3], sr_ratios=[8, 4, 2, 1], side_dims = 3):
        super().__init__()
        self.num_classes = num_classes
        self.depths = depths
        self.side_feature_early_fusion = side_feature_early_fusion
        ...
        # transformer encoder
        self.norm_add = norm_layer(embed_dims[0])
    ...
    def forward_features(self, x, additional_feature = None):
        ...
        if additional_feature is not None:
            ...
            embeds.append(af.reshape(B, H, W, -1).permute(0, 3, 1, 2).contiguous()) #1/4
        ...
        if additional_feature is not None and self.side_feature_early_fusion:
            x = x + af
            x = self.norm_add(x)
        ...
        outs.append(x)
        if additional_feature is not None and not self.side_feature_early_fusion:
            x = x + embeds[0]
    ...
    def forward_mix_features(self, x, mix_hooks, additional_feature = None):
...
class mit_b0(MixVisionTransformer):
    def __init__(self, side_dims, **kwargs):
        super(mit_b0, self).__init__(
            patch_size=4, embed_dims=[32, 64, 160, 256], num_heads=[1, 2, 5, 8], mlp_ratios=[4, 4, 4, 4],
            qkv_bias=True, norm_layer=partial(nn.LayerNorm, eps=1e-6), depths=[2, 2, 2, 2], sr_ratios=[8, 4, 2, 1],
            drop_rate=0.0, drop_path_rate=0.1, side_dims=side_dims)
