...
class HighResolutionNet(nn.Module):
    def __init__(self, width, num_classes, ocr_width=256, small=False,
        ...
        self.multi_scale_channels = pre_stage_channels
        last_inp_channels = np.int(np.sum(pre_stage_channels))
        self.backbone_dim = last_inp_channels
        if self.ocr_width > 0:
    ...
    def forward(self, x, additional_features=None, points=None, coord_dist_map=None):
        ...
        spatial_ratio = feats.shape[-1] / x.shape[-1]
        if self.ocr_width > 0:
    ...
    def compute_hrnet_multiscale_feats(self, x, additional_features):
        x = self.compute_pre_stage_features(x, additional_features)
        x = self.layer1(x)
