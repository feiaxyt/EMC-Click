...
class HRNetModel(ISModel):
    ...
    def __init__(self, width=48, ocr_width=256, small=True, backbone_lr_mult=0.1,
                 norm_layer=nn.BatchNorm2d, pipeline_version = 's1', has_refiner=True,
                 **kwargs):
    ...
    def forward(self, image, points, images_focus=None, points_focus=None, rois=None):
        ...
        if self.with_aux_output:
            outputs['instances_aux'] = nn.functional.interpolate(outputs['instances_aux'], size=image.size()[2:],
                                                             mode='bilinear', align_corners=True)
        if images_focus is None:
            return outputs
        bboxes = torch.chunk(rois,rois.shape[0],dim=0)
        refine_output = self.refine(images_focus, points_focus, outputs['feature'], outputs['instances'], bboxes)
