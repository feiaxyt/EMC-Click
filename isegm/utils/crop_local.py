...
def get_full_focus_crop(pred_mask, global_roi, clicks_list, ratio):
    y1,y2,x1,x2 = get_bbox_from_mask(pred_mask)
    y1,y2,x1,x2 = expand_bbox(pred_mask,y1,y2,x1,x2,ratio)
    points = np.array([click.coords_and_indx for click in clicks_list])[:, :2] # y, x

    min_y, max_y = points[:, 0].min(), points[:, 0].max()
    min_x, max_x = points[:, 1].min(), points[:, 1].max()
    x1 = min(max(min_x-1, 0), x1) #保证click要在roi内部
