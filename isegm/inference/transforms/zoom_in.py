...
class ZoomIn(BaseTransform):
    ...
    def inv_transform(self, prob_map):
        ...
        if self._prev_probs is not None:
            new_prob_map = torch.from_numpy(self._prev_probs).to(prob_map.device) #new_prob_map = torch.zeros(*self._prev_probs.shape, device=prob_map.device, dtype=prob_map.dtype)
