...
from .clickatt import ClickAttPredictor
...
def get_predictor(net, brs_mode, device,
    ...
    if brs_mode == 'NoBRS':
        ...
        predictor = BasePredictor(net, device, zoom_in=zoom_in, with_flip=with_flip, infer_size=zoom_in_params['target_size'] if 'target_size' in zoom_in_params else 448, **predictor_params_)
    ...
    elif brs_mode == 'EMC-Click':
        if predictor_params is not None:
            predictor_params_.update(predictor_params)
        predictor = ClickAttPredictor(net, device, with_flip=with_flip, infer_size=zoom_in_params['target_size'] if 'target_size' in zoom_in_params else 384, **predictor_params_)
